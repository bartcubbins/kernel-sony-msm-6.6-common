...
for platform in "${!PLATFORMS[@]}"; do
    if [ -z "${only_build_for:-}" ] || [ "$platform" = "${only_build_for:-}" ]; then
        ...
        find "$KERNEL_TMP_PLATFORM/arch/arm64/boot/dts/vendor/{qcom,somc}/" \( -name *.dtb -o -name *.dtbo \) -delete 2>/dev/null || true
        ...
        dtb="$KERNEL_TMP_PLATFORM/arch/arm64/boot/dts/vendor/qcom/$SOCDTB"
        ...
        for device in $DEVICES; do
            dtbo="$KERNEL_TMP_PLATFORM/arch/arm64/boot/dts/vendor/somc/${SOC}-${platform}-${device}_generic-overlay.dtbo"
